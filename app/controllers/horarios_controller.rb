class HorariosController < ApplicationController
  ...
  # GET /horarios.json
  def index
    @horarios = Horario.all.paginate(page:params[:page], per_page: 3);
  end
  ...
  private
    ...
    def horario_params
      params.require(:horario).permit(:hora, :Horario_id, :academia_id)
    end
    ...
    def listar
        ...
        respond_to do |format|
          format.html
          format.pdf do
          pdf = horariosReport.new(@horarios)
          send_data pdf.render, filename: 'horariosListagem.pdf', :width =>
          pdf.bounds.width,
