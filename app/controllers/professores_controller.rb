class ProfessoresController < ApplicationController
  ...
  def index
    ...
    @professores = Professor.where(filtro).order("nome").paginate(page:params[:page], per_page: 3);
  end
  ...
  private
    ...
    def professor_params
      params.require(:professor).permit(:nome, :telefone, :endereco, :cpf, :academia_id, :foto)
    end

    def listar
        @professores = Professor.all.paginate(page: params[:page], per_page: 3)
        respond_to do |format|
          format.html
          format.pdf do
          pdf = professoresReport.new(@professores)
          send_data pdf.render, filename: 'professoresListagem.pdf', :width =>
          pdf.bounds.width,
